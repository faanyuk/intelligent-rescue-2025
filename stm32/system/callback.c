...
void uart_show(void) {
    ...
    if (sys_tick % 1000 == 0) {
        sprintf(transmitData, "State:%d,%d,%d,%d\n实际数值：%.2f,%.2f,%d,%d\n", values.motor1_speed,
                values.motor2_speed, values.servo1_angle, values.servo2_angle, current_speed1,
                current_speed2, Get_Servo_Angle(1), Get_Servo_Angle(2));
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim == &htim1) {
        // 按键检测+0.01s软件消抖
        if (state_button1 == 0 && HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)
            state_button1 = 1;
        ...
        if (state_button2 == 0 && HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_14) == GPIO_PIN_RESET)
            state_button2 = 1;
        ...
        if (state_button3 == 0 && HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_15) == GPIO_PIN_RESET)
            state_button3 = 1;
