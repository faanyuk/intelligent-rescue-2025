...
from vision import VISION, VideoStream
...
def is_ball_in_holding_area(ball_center):
    ...
    x, y = ball_center
    return (
        HOLDING_AREA[0] <= x <= HOLDING_AREA[2]
        and HOLDING_AREA[1] <= y <= HOLDING_AREA[3]
    )
...
while running:
    ...
    if current_time > last_time + 0.5:
        ...
        print(
            f"第{frame_count}次运行。当前状态：{state_list[state]}，爪子状态：{claw_state}"
        )
            ...
            if ball_in_position:
                ...
                if first_catch:
                    ...
                    if current_ball_data["label"] != expected_color:
                        print(
                            f"错误：抓到的是{current_ball_data['label']}而不是"
                            f"{expected_color}，释放并重新寻找"
                        )
                        controller.release()
    ...
    if state == 2:
        ...
        if flag_found_area:
            ...
            # 检查安全区中心是否在指定区域
            if (
                CENTER_REGION[0] <= acx <= CENTER_REGION[2]
                and CENTER_REGION[1] <= acy <= CENTER_REGION[3]
            ):
    ...
    if state in [2, 3] and flag_found_ball and ball_data:
        ball_center = ball_data["center"]
        cv2.circle(
            frame, (int(ball_center[0]), int(ball_center[1])), 10, (0, 0, 255), -1
        )
